...
# filter js files (end with .js and not jquery files)
def js_file_filter(files):
    re = []
    for file in files:
        if file.endswith('.js') and 'jquery' not in file:
            if file.startswith('/'):
                file = file[1:]
            re.append(file)
    return re

# mark the APIs in files
# e.g. chrome.runtime.onMessage.addListener
def mark_msg_API( files, mark):
    originAPIs = ["chrome.runtime.onMessage.addListener"]
    for file in files:
        with open(file) as fin:
            content = fin.read()
            for originAPI in originAPIs:
                content = re.sub(
                    r'(' + originAPI + r')(?!' + mark + r')',
                    r'\1' + mark,
                    content)
        with open (file, 'w') as fout:
            fout.write(content)

def preprocess_and_get_entrance_files_of_extension(extension_path, ingore_JQuery = True):
    """
     preprocess and get the entrance file pathes of a extension
     Args:
         extension_path: the path of a extension_path
     return:
    ...
    with open(os.path.join(extension_path, 'manifest.json')) as f:
        ...
        if 'content_scripts' in manifest:
            if 'js' in manifest['content_scripts'][0]:
                csfiles = manifest['content_scripts'][0]['js']
                if ingore_JQuery:
                    csfiles = js_file_filter(csfiles)
                csfiles = [os.path.join(extension_path, i) for i in csfiles]
                mark_msg_API(csfiles, "_cs")
                files.extend(csfiles)
        if 'web_accessible_resources' in manifest:
            warfiles = manifest['web_accessible_resources']
            if ingore_JQuery:
                warfiles = js_file_filter(warfiles)
            warfiles = [os.path.join(extension_path, i) for i in warfiles]
            mark_msg_API(warfiles, "_war")
            files.extend(warfiles)
        if 'background' in manifest:
            if 'scripts' in manifest['background']:
                bgfiles = manifest['background']['scripts']
                if ingore_JQuery:
                    bgfiles = js_file_filter(bgfiles)
                bgfiles = [os.path.join(extension_path, i) for i in bgfiles]
                mark_msg_API(bgfiles, "_bg")
                files.extend(bgfiles)
                ...
                with open(os.path.join(extension_path, bgpage)) as f:
                    ...
                    if ingore_JQuery:
                        bgfiles = js_file_filter(scripts)
                    bgfiles = [os.path.join(extension_path, i) for i in bgfiles]
                    mark_msg_API(bgfiles, "_bg")
                    files.extend(bgfiles)
    return files
    ...
    #         content += fin.read()
    #         content += '\n'
    # GENERATED_FILE = os.path.join(extension_path, 'generated_file2parse.js')
