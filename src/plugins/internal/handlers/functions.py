...
class HandleASTCall(Handler):
    ...
    def process(self):
        ...
        res = NodeHandleResult(obj_nodes=r.obj_nodes, used_objs=r.used_objs,
            values=r.values, value_sources=r.value_sources,
            ast_node=self.node_id, callback=get_df_callback(self.G))
        # print('function call result:', self.node_id)
        # print(res)
        return res
...
def call_function(G, func_objs, args=[], this=NodeHandleResult(), extra=None,
    ...
    if G.opt_func and G.num_function_call[call_stack_item] > 5:
        ...
        # print(G.get_obj_def_ast_node(callee_ast))
        ...
        if python_func: # special Python function
            if is_new:
                if func_obj in G.builtin_constructors:
                    loggers.main_logger.log(ATTENTION, f'Running Python function to new {func_obj} {python_func}...')
                    try:
                ...
                # print(f'======debug Python function {python_func}...')
                try:
...
def simurun_function(G, func_ast, branches=None, block_scope=True,
    ...
    loggers.main_logger.info(sty.ef.inverse + sty.fg.green +
        ...
        branches) + sty.rs.all)
    '''
    print("======debug FUNCTION {} STARTS, SCOPE {}, DECL OBJ {}, this OBJs {}, branches {}"
        .format(func_name or '{anonymous}',
        G.cur_scope, func_obj, G.cur_objs,
        branches) + sty.rs.all)
    '''
...
def get_module_exports(G, file_path):
    from src.plugins.internal.handlers.file import run_toplevel_file
