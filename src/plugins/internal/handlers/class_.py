...
def handle_class(G: Graph, ast_node, extra):
    ...
    print(G.get_node_attr(children[4]))
    toplevel = children[4]
    edges = G.get_out_edges(toplevel, edge_type='PARENT_OF')
    print(edges)
...
def simurun_class_body(G, ast_node, extra):
    ...
    loggers.main_logger.info('BLOCK {} STARTS, Handling class, SCOPE {}'.format(ast_node, G.cur_scope))
...
def handle_method(G: Graph, ast_node, extra):
    ...
    from .func_decl import decl_function
