        ...
        for prop_name_node in G.get_prop_name_nodes(arr):
            ...
            try:
                ...
                if name=='length':
                    ...
                    length_obj = objs[0]
                    original_len = G.get_node_attr(length_obj).get('code')
                    if original_len==0:
                        returned_objs.update([G.undefined_obj])
                        break
                    G.set_node_attr(length_obj, ('code', original_len-1))
                    # print('length after shift:',original_len-1 )
                i = int(name)
                if i == 0:
                    objs = G.get_obj_nodes(prop_name_node, extra.branches)
