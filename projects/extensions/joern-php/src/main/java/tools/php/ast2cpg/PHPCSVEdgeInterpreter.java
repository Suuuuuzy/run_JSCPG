	...
	private int handleFunction( FunctionDef startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // name child
				...
				break;
			case 1: // docComment child: no support at present
				// if (endNode instanceof NullNode){
					startNode.addChild(endNode);
				// } else {
			...
			case 4: // returnType child: either Identifier or NULL node
				if (endNode instanceof NullNode){
					startNode.addChild(endNode);
				} else if (endNode instanceof StringExpression || endNode instanceof IntegerExpression || endNode instanceof DoubleExpression) {
					Identifier returnTypeNode = new Identifier();
					returnTypeNode.setLocation(endNode.getLocation());
					returnTypeNode.setCodeStr(endNode.getEscapedCodeStr());
					returnTypeNode.setProperty(PHPCSVNodeTypes.TYPE.getName(), PHPCSVNodeTypes.TYPE_NAME);
					returnTypeNode.setProperty(PHPCSVNodeTypes.CHILDNUM.getName(), endNode.getProperty(PHPCSVNodeTypes.CHILDNUM.getName()));
					returnTypeNode.setNodeId(endNode.getNodeId());
					StringExpression nameNode = new StringExpression();
					nameNode.setProperty(PHPCSVNodeTypes.TYPE.getName(), PHPCSVNodeTypes.TYPE_STRING);
					nameNode.setLocation(endNode.getLocation());
					nameNode.setCodeStr(endNode.getProperty(PHPCSVNodeTypes.TYPE.getName()));
					nameNode.setProperty(PHPCSVNodeTypes.CHILDNUM.getName(),"0");
					returnTypeNode.setNameChild(nameNode);
					startNode.setReturnType(returnTypeNode);
				} else {
					startNode.addChild(endNode);
				}
				break;
			case 5: // __declId node: no suppport at present
				startNode.addChild(endNode);
				break;
	...
	private int handleClosure( Closure startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			...
			case 2: // params child
				startNode.setParameterList((ParameterList)endNode);
				break;
			case 3: // uses child: no support at present
				startNode.addChild(endNode);
				break;
			case 4: // stmts child
				startNode.setContent((CompoundStatement)endNode);
	...
	private int handleClass( ClassDef startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			...
			case 1: // docComment child: no support at present
				startNode.addChild(endNode);
				break;
			case 2: // extends child: either Identifier or NULL node
				if( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
					startNode.setExtends((Identifier)endNode);
				break;
			case 3: // implements child: either IdentifierList or NULL node
				if( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
	...
	private int handleForEach( ForEachStatement startNode, ASTNode endNode, int childnum)
	{
		...
		switch (childnum)
		{
			case 0: // expr child: Expression node
				...
				break;
			case 1: // value child: Expression or null node
				// PHP always has value child. key child is optional.
				// JavaScript has either value or key child. Both children won't occur at the same time.
				if( endNode instanceof NullNode)
					startNode.addChild(endNode);
				else
