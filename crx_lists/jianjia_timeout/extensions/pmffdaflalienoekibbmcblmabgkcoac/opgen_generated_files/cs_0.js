...
// original file:/Users/jianjia/Documents/tmp/EOPG/run_JSCPG_merge/crx_lists/jianjia_timeout/extensions/pmffdaflalienoekibbmcblmabgkcoac/add_nbviewer_button.js
...
function listener(){
  ...
  /* Find the first lot of aui-buttons in the toolbar */
  var toolbar_buttons = $($(".toolbar").find(".secondary").find(".aui-buttons")[0]);

  if (toolbar_buttons.children().size() == 3) {
    toolbar_buttons.children().each(function () {
      /* Go through the buttons on the toolbar.
         Extract the href from the "Raw" button.
         Add new button with link to NBviewer, using the Raw href */
      if($(this).text() == 'Raw') {
        if ($(this).attr("href").indexOf(".ipynb") >= 0 ) {
          toolbar_buttons.append('<a href=\"http://nbviewer.ipython.org/urls/bitbucket.org/' + $(this).attr("href") + '\" class=\"aui-button aui-button-light\">NBView</a>')
          console.log("BITBUCKET_NBVIEW: Added NBView button.");
        }
      }
    })
  }
...
/* Any time we navigate, check to see if the content has changed */
$('#content').bind('DOMSubtreeModified',listener);
