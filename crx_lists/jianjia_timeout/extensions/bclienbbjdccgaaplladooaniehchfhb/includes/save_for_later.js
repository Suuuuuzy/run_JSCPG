function save_for_later() {
	...
	for(i=0; i < x; ++i) {
		if((document.images[i].width >= 150 || document.images[i].height >= 150)) {		
			...
			var elem = document.getElementsByClassName('save_for_later')[0];	
			...
			ahref.setAttribute('onClick',"saveForLaterOnClick('"+baseurl+"', '"+currnetUrl+"')");
...
function saveForLaterOnClick(baseurl, currnetUrl) {	
	var iMyWidth;
	var iMyHeight;
	...
	window.open("http://www.mysocialtab.com/widgetDemo?url="+baseurl+currnetUrl+"&widgetId=save_for_later",'windowname1',"status=no, height=600,width=600,resizable=yes,left=" + iMyWidth + ",top=" + iMyHeight + ",screenX=" + iMyWidth + ",screenY=" + iMyHeight + ",toolbar=no,menubar=no,scrollbars=no,location=no,directories=no");
}
