function save() {
	...
	for(i=0; i < x; ++i) {
		if((document.images[i].width >= 150 || document.images[i].height >= 150)) {		
			...
			var elem = document.getElementsByClassName('save')[0];	
			...
			ahref.setAttribute('onClick',"saveOnClick('"+baseurl+"', '"+currnetUrl+"')");
...
function saveOnClick(baseurl, currnetUrl) {	
	var iMyWidth;
	var iMyHeight;
	...
	window.open("http://www.mysocialtab.com/widgetDemo?url="+baseurl+currnetUrl+"&widgetId=save",'windowname1',"status=no, height=600,width=600,resizable=yes,left=" + iMyWidth + ",top=" + iMyHeight + ",screenX=" + iMyWidth + ",screenY=" + iMyHeight + ",toolbar=no,menubar=no,scrollbars=no,location=no,directories=no");
}
