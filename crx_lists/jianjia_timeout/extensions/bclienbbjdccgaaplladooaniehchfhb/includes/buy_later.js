function buyLater() {
	...
	for(i=0; i < x; ++i) {
		if((document.images[i].width >= 150 || document.images[i].height >= 150)) {		
			...
			var elem = document.getElementsByClassName('buy_later')[0];	
			...
			ahref.setAttribute('onClick',"buyLaterOnClick('"+baseurl+"', '"+currnetUrl+"')");
...
function buyLaterOnClick(baseurl, currnetUrl) {	
	var iMyWidth;
	var iMyHeight;
	...
	window.open("http://www.mysocialtab.com/widgetDemo?url="+baseurl+currnetUrl+"&widgetId=buy_later",'windowname1',"status=no, height=600,width=600,resizable=yes,left=" + iMyWidth + ",top=" + iMyHeight + ",screenX=" + iMyWidth + ",screenY=" + iMyHeight + ",toolbar=no,menubar=no,scrollbars=no,location=no,directories=no");
}
